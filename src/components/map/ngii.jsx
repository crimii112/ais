...
const Ngii = ({ SetMap }) => {
  ...
  const layerGnrl = new VectorLayer({
    source: sourceGnrl,
    id: 'gnrl',
    zIndex: 10,
  });
  ...
  const layerPrtr = new VectorLayer({
    source: sourcePrtr,
    id: 'prtr',
    zIndex: 9,
  });
  let layerPrtrHeatmap = new HeatmapLayer({
  ...
  const layerSems = new VectorLayer({
    source: sourceSems,
    id: 'sems',
    style: null,
    zIndex: 9,
  });
  let layerSemsHeatmap = new HeatmapLayer({
  ...
  let layerCapssM = new VectorImageLayer({
    source: sourceCapssM,
    id: 'capss_m',
    declutter: true,
    renderMode: 'webgl',
  ...
  let layerCapssMHeatmap = new HeatmapLayer({
  ...
  let layerLanduse = new VectorImageLayer({
    source: sourceLanduse,
    id: 'landuse',
    declutter: true,
    renderMode: 'webgl',
    style: null,
  ...
  const layerMoctLink = new VectorImageLayer({
    source: sourceMoctLink,
    id: 'moct_link',
    declutter: true,
    renderMode: 'webgl',
    style: null,
  ...
  const layerPpltndn = new VectorLayer({
    source: sourcePpltndn,
    style: null,
    id: 'ppltndn',
    zIndex: 3,
  });
  ...
  const layerSearchedSites = new VectorLayer({
    source: sourceSearchedSites,
    id: 'SearchedSites',
    style: null,
    zIndex: 10,
  });
  ...
  const layerSearchRadius = new VectorLayer({
    source: sourceSearchRadius,
    id: 'SearchRadius',
    style: null,
    zIndex: 9,
  });
  ...
  const layerDistance = new VectorLayer({
  ...
  const SetMarker = async coord => {
    ...
    if (window.chrome.webview) {
      ...
      window.chrome.webview.postMessage({
        type: 'mapClick',
        data: {
          wgs: `${wgs[0].toFixed(5)} ${wgs[1].toFixed(5)}`,
          utm: `${parseFloat(coord[0]).toFixed()} ${parseFloat(
            coord[1]
          ).toFixed()}`,
          tm: `${tm[0].toFixed()} ${tm[1].toFixed()}`,
          grs: `${parseFloat(grs[0]).toFixed()} ${parseFloat(
            grs[1]
          ).toFixed()}`,
          addr: addStr,
        },
  ...
  const handleWindowMessage = async e => {
    ...
    if (message.pagetype === 'layerVisible') {
      ...
      if (message.visibleType === 'site') {
        ...
        map.getLayers().forEach(layer => {
          ...
          if (
            layer.get('id').toLowerCase() === message.visibleType.toLowerCase()
          ) {
    ...
    if (
      message.pagetype.includes('site') &&
      !message.pagetype.includes('searchradius')
    ) {
      ...
      let chartIndex = map
        .getAllLayers()
        .findIndex(layer =>
          layer.get('id') ? layer.get('id') === 'Chart' : -1
        );
      let layer;
      ...
      Object.keys(message.charts).forEach(key => {
        ...
        layer
          .getSource()
          .addFeatures(gsonFormat.readFeatures(message.charts[key].value));
      });
  ...
  const StylingLayers = (obj = {}) => {
    Object.keys(obj).forEach(key => {
      ...
      if (key.includes('chart')) {
        ...
        const pieStyle =
          arrStyles[arrStyles.findIndex(item => item.charttype === 'pie')];
        const barStyle =
          arrStyles[arrStyles.findIndex(item => item.charttype === 'bar')];

        layerTarget.setStyle(feature => {
          let style;
          ...
          if (feature.get('chart_type') === 'pie') {
            style = new Style({
              image: new Chart({
                ...
                displacement: [
                  parseInt(pieStyle.display_x),
                  parseInt(pieStyle.display_y),
                ],
                rotateWithView: true,
                stroke: new Stroke({
                  color: pieStyle.stroke_color,
                  width: parseInt(pieStyle.stroke_width),
                }),
              }),
            });
            ...
            style = new Style({
              image: new Chart({
                ...
                displacement: [
                  parseInt(barStyle.display_x),
                  parseInt(barStyle.display_y),
                ],
                rotateWithView: true,
                stroke: new Stroke({
                  color: barStyle.stroke_color,
                  width: parseInt(barStyle.stroke_width),
                }),
              }),
            });
          }
        ...
        if (key === 'gnrl') {
          ...
          const keyKo =
            key === 'prtr' ? 'PRTR' : key === 'sems' ? 'SEMS' : '인구밀도';
          ...
          styles.forEach(step => {
            const filter = [
              'all',
              ['<=', parseFloat(step.min), ['get', pollType]],
              ['>=', parseFloat(step.max), ['get', pollType]],
            ];
          ...
          obj[key].styles.forEach(step => {
            if (step.stroke_color || step.fill_color) {
              if (
                !(step.stroke_color === 'null' || step.fill_color === 'null')
              ) {
                arrStyles.push({
  ...
  return (
    <Container id="ngii">
      ...
      <DivControlLegend
        className="gis-legend-container ol-legend ol-legend-right"
        id="gisLegends"
      ></DivControlLegend>
    </Container>
  );
