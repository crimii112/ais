...
const ContentGis = ({ SetMap, sitetype, tms, onAddStation, onInit }) => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      controls.forEach(control => {
        if (
          control.element &&
          control.element.className === 'radius-control-container'
        ) {
  ...
  const handleMapPointermove = e => {
    ...
    if (!feature) {
      return;
    } // 이거 없으면 그냥 지도 위에 마우스 올리면 아래 함수가 작동됨. 지도 자체도 오브젝트이기때문
        ...
        .then(data => {
          ...
          searchList.forEach(item => {
            ...
            const siteData =
              feature.get('site_cd') +
              ';[' +
              feature.get('area_type2') +
              ']' +
              feature.get('gungu_nm') +
              '.' +
              feature.get('site_nm');
            const siteJson = {
              siteCd: Number(feature.get('site_cd')),
              siteData: siteData,
            };
