...
const IntensivePieGraph = () => {
    ...
    const handleClickDrawChart = () => {
        const chartData = contentData.rstList.filter(item => item.groupdate === chartSelectedOption.value.split(';')[0] && item.groupNm === chartSelectedOption.value.split(';')[1]);
        setChartDatas({ rstList: chartData });

        const axisSettings = [{ 
            label: '물질', 
            selectedOptions: [{value: 'amSul', text: 'AM_SUL'}, {value: 'amNit', text: 'AM_NIT'}, {value: 'om', text: 'OM'},
                {value: 'ec', text: 'EC'}, {value: 'etc', text: 'ETC'}] 
        }];
            ...
            {isLoading ? (
                ...
                <FlexColWrapper className='w-full h-full gap-15'>
                    <FlexRowWrapper className="w-full gap-10 mb-4 items-stretch justify-between">
                        ...
                        <span className="flex flex-col items-center justify-center">측정일자;측정소 : </span>
                            <SelectWithArrows
                                id="groupDateAndNm"
