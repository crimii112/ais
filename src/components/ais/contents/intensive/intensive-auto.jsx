...
const IntensiveAuto = ({ type }) => {
    ...
    const [chartOptionSettings, setChartOptionSettings] = useState({pollutant: [], groupNm: []});
    ...
    const handleChangeChartSelectedOption = e => {
        const selectedOption = chartOptionSettings.pollutant.find(option => option.value === e.target.value);
        setChartSelectedOption(prev => ({ ...prev, [e.target.id]: selectedOption }));
    }
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload?.length) {
      ...
      return (
        <div className="bg-white p-2.5 border-1 border-gray-300 rounded-md">
          <p className="font-semibold pb-1">{groupdate} - {groupNm}</p>
          <p>{`${xLabel} : ${x}`}</p>
          <p>{`${yLabel} : ${y}`}</p>
        </div>
      );
    ...
    const handleClickDrawChart = () => {
        ...
        if (!rawData.length) return alert('그래프를 그릴 데이터가 없습니다. 조건을 확인해주세요.');
    ...
    return (
        <>
            ...
            <ContentScatterChartFrame isLoading={isLoading} title={config.title} chartSettings={chartSettings}>
